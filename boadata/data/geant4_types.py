...
class Geant4Scoring(XarrayDatasetBase):
    ...
    def from_uri(cls, uri, box_size=None, translate=None, *args, **kwargs):
        ...
        for scorer in scorers:
            scorer_df = pd.read_csv(
                uri,
                skiprows=scorer[1],
                names=["ix", "iy", "iz", scorer[0]],
                index_col=[0, 1, 2],
        ...
        if box_size and translate:
            data["ix"] = (data["ix"] - len(data["ix"]) / 2.0 + 0.5) * 2 * box_size[
                0
            ] / (len(data["ix"])) + translate[0]
            data["iy"] = (data["iy"] - len(data["iy"]) / 2.0 + 0.5) * 2 * box_size[
                1
            ] / (len(data["iy"])) + translate[1]
            data["iz"] = (data["iz"] - len(data["iz"]) / 2.0 + 0.5) * 2 * box_size[
                2
            ] / (len(data["iz"])) + translate[2]
